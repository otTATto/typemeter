...
起動時の初期状態は起動経路によって異なります。通常起動ではメインウィンドウを表示しますが、OS ログイン時の自動起動（Settings の「Auto Launch at PC Login」、`--hidden` 引数付きで起動される）では、ウィンドウを表示せず最初から常駐中の状態で開始します（macOS では Dock にも出ません）。
