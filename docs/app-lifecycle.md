...
stateDiagram-v2
    表示中 --> 常駐中: メインウィンドウの ×
    常駐中 --> 表示中: トレイ左クリック（Windows）/ メニューの Open typemeter / 再起動（二重起動検知）
...
typemeter は単一インスタンスで動作し、2 個目の起動は「既存プロセスのメインウィンドウを表示する」操作に変換されます（`tauri-plugin-single-instance`）。

常駐中はウィンドウが見えないため、「起動していないと思って再度起動する」操作が起こりやすくなります。これを放置すると計測プロセスが二重になりカウントが重複するため、プラグインが 2 個目のプロセスを即座に終了させ、代わりに既存プロセスへ通知してメインウィンドウを表示します。ユーザーから見ると「アプリを起動したらウィンドウが出てきた」という自然な挙動になります。
