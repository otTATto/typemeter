...
現在の設定項目は「常に最前面に表示」と「ライト / ダークテーマ」の 2 つです。

| 設定項目              | 設定場所                                       | `settings.json` のキー | 型                  | キー不在時の挙動              |
| --------------------- | ---------------------------------------------- | ---------------------- | ------------------- | ----------------------------- |
| 常に最前面に表示      | メインウィンドウのヘッダー右側（ピン型トグル） | `alwaysOnTop`          | `boolean`           | `false`（最前面に固定しない） |
...
このように、頻繁に切り替える「常に最前面に表示」はメインウィンドウに直接置かれ、それ以外の設定は Settings ウィンドウに集約されています。
...
以上のとおり、設定の実体はアプリデータディレクトリの設定ファイル 1 つであり、フロントエンドは `src/lib/settings.ts` を経由してのみアクセスします。
...
どのウィンドウで設定を変更しても、ストアの変更通知（`onKeyChange`）を通じて全ウィンドウに反映されます。
...
- `alwaysOnTop`: メインウィンドウのトグル（`AlwaysOnTopToggle`）が、ストアへの保存前に `getCurrentWindow().setAlwaysOnTop()` でウィンドウへ直接適用します。保存に失敗した場合はウィンドウ状態・トグル表示とも元の状態にロールバックします
